<? get_header() ?>
<div class="col-xs-12">
	<div class="page__wrapper">
		<div class="page__head">
			<h3>Путешествия / Полинезия</h3>
		</div>
		<div class="page__body page__body--wrap page__scrolltext single-category">

			<a href="#" class="single-category__item">
				<span class="single-category__item--img">
					<img class="" src="<?php bloginfo('template_directory'); ?>/img/@Elf@.jpg" alt="">
				</span>
				<span class="single-category__item--desc">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Omnis accusantium autem, qui illo quidem ipsum!</span>
			</a>
			<a href="#" class="single-category__item">
				<span class="single-category__item--img">
					<img class="" src="<?php bloginfo('template_directory'); ?>/img/photo2.jpg" alt="">
				</span>
				<span class="single-category__item--desc">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Omnis accusantium autem, qui illo quidem ipsum!</span>
			</a>
			<a href="#" class="single-category__item">
				<span class="single-category__item--img">
					<img class="" src="<?php bloginfo('template_directory'); ?>/img/photo3.jpg" alt="">
				</span>
				<span class="single-category__item--desc">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Omnis accusantium autem, qui illo quidem ipsum!</span>
			</a>
			<a href="#" class="single-category__item">
				<span class="single-category__item--img">
					<img class="" src="<?php bloginfo('template_directory'); ?>/img/photo4.jpg" alt="">
				</span>
				<span class="single-category__item--desc">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Omnis accusantium autem, qui illo quidem ipsum!</span>
			</a>
			<a href="#" class="single-category__item">
				<span class="single-category__item--img">
					<img class="" src="<?php bloginfo('template_directory'); ?>/img/photo5.jpg" alt="">
				</span>
				<span class="single-category__item--desc">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Omnis accusantium autem, qui illo quidem ipsum!</span>
			</a>
			<a href="#" class="single-category__item">
				<span class="single-category__item--img">
					<img class="" src="<?php bloginfo('template_directory'); ?>/img/photo6.jpg" alt="">
				</span>
				<span class="single-category__item--desc">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Omnis accusantium autem, qui illo quidem ipsum!</span>
			</a>
			<a href="#" class="single-category__item">
				<span class="single-category__item--img">
					<img class="" src="<?php bloginfo('template_directory'); ?>/img/photo7.jpg" alt="">
				</span>
				<span class="single-category__item--desc">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Omnis accusantium autem, qui illo quidem ipsum!</span>
			</a>
			<a href="#" class="single-category__item">
				<span class="single-category__item--img">
					<img class="" src="<?php bloginfo('template_directory'); ?>/img/photo8.jpg" alt="">
				</span>
				<span class="single-category__item--desc">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Omnis accusantium autem, qui illo quidem ipsum!</span>
			</a>
			<div class="preloader"></div>
		</div>
	</div>
</div>
<?get_footer()?>